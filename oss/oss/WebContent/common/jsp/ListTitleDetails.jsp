...
<c:url var="listUrl" value="ListTitleDetails.jsp">
...
<c:catch var="error">
	<c:forEach var="titleDetails" items="${listTitles.titles}">		
        <TR>
        	...
        	<c:set var="pricings" value="${titleDetails.pricings}"/>
        	<TD>
    	    ...
    	    <TD colspan=2>
    	    	...
    	    	<c:forEach var="pricing" items="${pricings}">
    	    		<TR>
    	    		<TD>
    	    		<c:if test="${not empty pricing.licenseType}">
...
<BR>
	<button class="tdbutton" onclick="window.location='<c:out value="${sortUrl}" />'">
	Sort
    </button>
<BR><BR>
